...
\sectionAndLabel{\code{@Tainted} checker string arguments}{tainting-string-arguments}

The \code{@Tainted} and \code{@Untainted} annotations are too generalized to be used for
specialized data types. Hence, for specialized semantics and for annotating for multiple types, we can add
a string array argument to the annotations, e.g. \code{@Tainted("OS")} and \code{@Untainted({"SQL", "HTML"})}.
The array arguments describe the types for which the object is either tainted or untainted, which can be then be used
to ensure safe data usage. For example, to prevent SQL injections attacks, we can make sure that the argument passed
is \code{@Untainted("SQL").


\sectionAndLabel{Indirect information flow}{indirect-information-flow}

The tainting checker can also aid the user in securing sensitive data and prevent it from leaking. Providing
the '-Aflow' option will enable the Tainting Checker to check for instances of indirect information flow. Consider
...
One can observe that the secure Social Security Number will be leaked into the unsecure 'i' variable. This is due
to the comparison condition in the while loop where a tainted variable is compared with an untainted one. The
Tainting Checker searches for such comparisons and raises an error when the '-Aflow' option is provided.
