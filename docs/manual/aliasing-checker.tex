...
There are four possible types for an expression:
...
\item[\refqualclass{common/aliasing/qual}{Linear}] is the type of an expression that can only be operated upon once. After this operation, it becomes @Unusable.
@Linear objects cannot have aliases and can accommodate only one method invocation.

\item[\refqualclass{common/aliasing/qual}{Unusable}] is the type of an expression thatcan not be either assigned, referenced or operated upon again.
Doing so would invoke the [use.unsafe] error in the compiler. This annotation is, however, hidden and cannot be used by the user explicitly.
...
The \code{@Linear} and \code{@Unusable} annotations implement type-checking for a linear type system.  A
...
Figure~\ref{fig-linear-example} gives an example.
